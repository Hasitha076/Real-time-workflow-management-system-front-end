...
export default function OTPInput({option, setShowNewPassword, setShowOTP, setResetDisabled}) {
  ...
  const sendOtp = async (value) => {
    const otp = parseInt(value.split('').join(''));
    if(option === otp) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
      ...
      <div 
        button={true}
        activeButton={true}
        style={{ marginTop: "6px", marginBottom: "24px", backgroundColor: "#854CE6", color: "#fff", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", textAlign: "center" }}
        onClick={() => sendOtp(otp)}
        >
        Send OTP
        </div>
